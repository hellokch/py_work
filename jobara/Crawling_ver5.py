...
stopwords = ['그리고', '하지만', '그런데', '따라서', '그래서', '또한', '이러한', '바로', '이런', '결국',
             '합니다','했습니다','입니다', '그것', '그때', '그들', '그것들', '그리고', '그만큼', '그러나', '그래서', 
             '그런데', '그렇게', '그리하여', '그것으로', '그중에서', '그대로', '그리고도', '그것만으로', 
             '그래도', '그것들의', '그럼에도 불구하고', '그것과 같은', '그와 같은',  '그는', 
             '그때에', '그러한', '그저', '그녀를', '그녀의', '그들의', '그런', '그에', '그럼으로써'
             '하다', '되다', '있다', '않다',"있는", "그", "할", "이", "것", "하다", "그것", "이런", "하는", 
...
#토크나이징 및 불용어 제거
def tokenize(text):
    words = okt.morphs(text, stem=True) # 형태소 분석을 수행합니다.
    words = [word for word in words if word not in stopwords] # stopwords를 제거합니다.
    return words
